...
module.exports = async (request, response, route) => {
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    if (route.isAdmin === true) {
      const bundles = readdirSync(
        path.resolve(CONSTANTS.ROOTPATH, './.redwireless/build/admin', route.id),
        { withFileTypes: true }
      )
  ...
  if (
    existsSync(path.resolve(CONSTANTS.ROOTPATH, './.redwireless/build', scopePath))
  )
    await rmdir(
      path.resolve(CONSTANTS.ROOTPATH, './.redwireless/build', scopePath),
      { recursive: true }
    );
